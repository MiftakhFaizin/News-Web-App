...
const CovidPage = () => {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                setLoading(true)
                const res = await fetch(`https://api.nytimes.com/svc/search/v2/articlesearch.json?q=covid&api-key=${apiKey}`)
    ...
    return (
        <div>
            <div className="py-[50px] border-b border-slate-500">
                <p className="text-[20px] font-bold text-center">COVID-19 NEWS</p>
            </div>
